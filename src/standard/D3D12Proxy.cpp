...
// BLINDAGEM: Usamos void* para pVideoMemoryInfo e para o dispositivo para o compilador n??o reclamar de tipos ausentes no SDK
...
// SPOOF: Interceptador do or??amento de mem??ria de v??deo (Estrutura mapeada manualmente via ponteiro de uint64)
static HRESULT STDMETHODCALLTYPE HookedQueryVideoMemoryInfo(
    ...
    // Mapeamento direto na mem??ria da estrutura D3D12_QUERY_VIDEO_MEMORY_INFO
    ...
    if (InterlockedCompareExchange(&g_loggedVideoMemoryInfo, 1, 0) == 0) {
        Log("SPOOF: QueryVideoMemoryInfo interceptado! For??ando 4GB VRAM Budget.");
    }
    return S_OK;
}
...
static void PatchDeviceInterfaces(IUnknown* deviceUnknown) {
    ...
    if (InterlockedCompareExchange(&g_isPatched, 1, 0) != 0) {
        if (InterlockedCompareExchange(&g_loggedPatchAlready, 1, 0) == 0) {
            Log("PatchDeviceInterfaces: j?? aplicado (ignorado)");
        }
        return;
    }
    ...
    if (!object) {
        Log("PatchDeviceInterfaces: ponteiro de objeto inv??lido");
        device->Release();
        return;
    }
    ...
    void** vtable = *object;
    if (!vtable) {
        Log("PatchDeviceInterfaces: vtable inv??lida");
        device->Release();
        return;
    }
    ...
    __try {
        ...
        if (currentCheckFeature == reinterpret_cast<void*>(&HookedCheckFeatureSupport)) {
            Log("PatchDeviceInterfaces: CheckFeatureSupport j?? patchado");
        } else {
            ...
            if (VirtualProtect(&vtable[checkFeatureIndex], sizeof(void*), PAGE_EXECUTE_READWRITE, &oldProtect)) {
                ...
                Log("PatchDeviceInterfaces: CheckFeatureSupport aplicado via probing din??mico");
            } else {
        ...
        if (currentQueryVidMem == reinterpret_cast<void*>(&HookedQueryVideoMemoryInfo)) {
            Log("PatchDeviceInterfaces: QueryVideoMemoryInfo j?? patchado");
        } else {
            if (!g_originalQueryVideoMemoryInfo) {
            ...
            if (VirtualProtect(&vtable[queryVideoMemoryIndex], sizeof(void*), PAGE_EXECUTE_READWRITE, &oldProtect)) {
                ...
                Log("PatchDeviceInterfaces: QueryVideoMemoryInfo aplicado via probing din??mico");
            } else {
                Log("PatchDeviceInterfaces: VirtualProtect falhou para QueryVideoMemoryInfo");
            }
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        Log("PatchDeviceInterfaces: exce????o capturada durante patch de vtable");
    }
